...
export default function ProjectBoard(): React.JSX.Element {
  return (
    <section
      style={style}
      className='grid lg:flex lg:justify-center lg:items-center bg-slate-100 relative py-24'
      id='project-board'>
      <div style={left} className='grid justify-items-center items-center'>
        <Comment>{'지금까지 제가 해온 프로젝트입니다.<br />여러가지를 시도해보았어요!'}</Comment>
      </div>
      <div style={right} className='grid justify-items-center items-center gap-12 relative'>
        <div className='absolute grid gap-8 -mt-36'>
          <ObservedDistributor prefix='project-card' animation='_anime-slp-ltr-slide' threshold={0.1}>
            <ProjectCard
              title='웹 포트폴리오'
